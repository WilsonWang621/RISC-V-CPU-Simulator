...
bool LoadStoreQueue::matching_response(const LSQEntry& entry, const DataMemoryResponse& response){
...
LSQOutputs LoadStoreQueue::evaluate(const LSQInputs& inputs){
    ...
    if(inputs.load_result_granted && cur_load_result_.valid){
        next_load_result_ = FUResult{};
    }
